...
from st_aggrid import AgGrid   # for nicer DF and adjustable DF column size
...
with header:
    ...
    st.markdown("* This tool will mine the web via three APIs (NEWS API, FINVIZ, and GoogleNews) to extract and gather financial news about a target company on a specific day.") 
    st.markdown("* It will then do sentiment analysis on news headlines to predict the tone (percent of positive, negative, and neutral).")
    st.markdown("* Lastly, it will extract important company events from the news texts, e.g. new products to launch, merger, acquisition, stock buybacks or splits, hiring, or lay off.")
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- 
    
    
with import_data:
    st.header('Import Data from NEWS API, FINVIZ, and GoogleNews APIs')
...
with news_headline_SA:
    ...
    fig = px.histogram(df_news,
                       ...
                       color_discrete_sequence=[
                           "silver", "mediumspringgreen", "orangered"
...
with kwrd_extract:
    ...
    fig = px.histogram(
        ...
        x='event_keywords',
        color='event_keywords').update_xaxes(categoryorder="total descending")
